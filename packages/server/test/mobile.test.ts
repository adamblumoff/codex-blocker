...
describe("extension pairing manager", () => {
  it("creates and confirms one-time pairing codes", () => {
    let now = 1_000;
    const pairing = new ExtensionPairingManager(() => now);
  ...
  it("reuses the active pairing code until expiration", () => {
    let now = 1_000;
    const pairing = new ExtensionPairingManager(() => now);
...
describe("mobile qr pairing manager", () => {
  it("creates and confirms one-time qr nonces", () => {
    let now = 1_000;
    const pairing = new MobileQrPairingManager(() => now);
    const created = pairing.startPairing();
    ...
    expect(pairing.confirmPairingQrNonce("bad-nonce")).toBe(false);
    expect(pairing.confirmPairingQrNonce(created.qrNonce)).toBe(true);
    expect(pairing.getStatus().active).toBe(false);
    expect(pairing.confirmPairingQrNonce(created.qrNonce)).toBe(false);
  });

  it("expires pairing window after ttl", () => {
    let now = 1_000;
    const pairing = new MobileQrPairingManager(() => now);
    const created = pairing.startPairing();
  ...
  it("reuses qr nonce when refresh is not requested", () => {
    let now = 1_000;
    const pairing = new MobileQrPairingManager(() => now);
    const first = pairing.startPairing();
    now = 1_001;
  ...
  it("refreshes qr nonce when requested", () => {
    let now = 1_000;
    const pairing = new MobileQrPairingManager(() => now);
    const first = pairing.startPairing();
    now = 1_001;
  ...
  it("rejects expired qr nonce and accepts fresh qr nonce once", () => {
    ...
    const refreshed = pairing.startPairing(true);
